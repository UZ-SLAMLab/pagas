...
if __name__ == '__main__':
    ...
    if args.mode == 'mesh':
        ...
        with mp.Pool(processes=30) as mp_pool:
            new_pts = mp_pool.map(sample_single_tri, ((n1[i,0], n2[i,0], v1[i:i+1], v2[i:i+1], tri_vert[i:i+1,0]) for i in range(len(n1))), chunksize=2048)

        new_pts = np.concatenate(new_pts, axis=0)
        data_pcd = np.concatenate([vertices, new_pts], axis=0)
        print(f"Number of 3D points extracted from mesh: {data_pcd.shape}")
