...
def cull_scan(scan, mesh_path, result_mesh_file, instance_dir, mask_cull=False):
    ...
    # load mask
    mask_dir = '{0}/masks_uncropped'.format(instance_dir)
    mask_paths = sorted(glob.glob(os.path.join(mask_dir, "*.png")))
    assert len(mask_paths) == n_images, f"Number of masks {len(mask_paths)} does not match number of images {n_images}."
    masks = []
    for p in mask_paths:
        mask = cv2.imread(p)
        masks.append(mask)
    ...
    if mask_cull:
        ...
        for i in tqdm(range(n_images),  desc="Culling mesh given masks"):
            ...
            intrinsic = intrinsics_all[i].to(device)
            ...
            with torch.no_grad():
                ...
                maski = torch.from_numpy(binary_dilation(maski, disk(24))).float()[None, None].to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_folder', type=str, required=True, help='path to the folder containing the DTu dataset')
    ...
    cull_scan(scan, ply_file, result_mesh_file, instance_dir=os.path.join(args.data_folder, f'scan{args.scan_id}'), mask_cull=True)
