...
# Metric functions from Marigold: https://github.com/prs-eth/Marigold/blob/main/src/util/metric.py
import torch
...
def abs_relative_difference(output, target, valid_mask=None, mean=True):
    ...
    return torch.sum(abs_relative_diff, (-1, -2)) / n if mean else abs_relative_diff
...
METRIC_FUNCS = [globals()[met] for met in EVAL_METRICS]


def eval_depth(depth: Tensor, gt_depth: Tensor, mask: Tensor):
    """Evaluation of the rendered depth in the target view against the grount truth depth."""
    metric_values = []
    for met_func in METRIC_FUNCS:
        _metric_name = met_func.__name__
        _metric = met_func(depth, gt_depth, mask).item()
        metric_values.append(_metric)

    return metric_values


def eval_normal(normal: Tensor, gt_normal: Tensor, mask: Tensor):
    """Compute the dot product distance between two normal vector tensors."""
    normal_error = ((1 - torch.sum(normal * gt_normal, dim=-1)) * 0.5)[mask].mean()
